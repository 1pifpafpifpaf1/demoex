...
namespace demoex
{
    public partial class Uvolit : MetroFramework.Forms.MetroForm
    {
        ...
        private void Uvolit_Load(object sender, EventArgs e)
        {
            GetComboBoxList();
        }
        ...
        void GetComboBoxList()
        {
            ...
            uvolbox.DataSource = list_eq_table;
            uvolbox.DisplayMember = "id";
            uvolbox.ValueMember = "id";
        ...
        {
            DB.OpenConnection();
            int id = Convert.ToInt32(uvolbox.SelectedValue);
            string status = Convert.ToString(statusbox.SelectedItem);
            string update = $"UPDATE users SET work = '{status}' WHERE id = {id}";
